...
* Constructor
*********************************************************************/
Sphere::Sphere(GLfloat radius, GLint resolution, GLuint numSegments)
{
    ...
    // Number of vertices in the Sphere
    ...
    // Number of faces in the Sphere (triangles, not squares)
    numFaces = resolution * 8 * numSegments;

    // Allocate memory for the Sphere faces
    ...
    for (GLuint curSegment = 0; curSegment < numSegments; curSegment++)
    {
        ...
        y = -radius + ((GLfloat) curSegment / (numSegments-1) * radius * 2);
    ...
    pMaterialEntryList[0]->pMaterial = new Material("Sphere");
    pMaterialEntryList[0]->pMaterial->iIllum = 0;
    numMaterials = 1;
};
